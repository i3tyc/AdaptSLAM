...
{
///////////////////////////////CommSLAM//////////////////////////////////////////////////
LoopClosing::LoopClosing(Atlas *pAtlas, Uncertainty *pUncertainty, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, string RunType, const bool bFixScale, const bool bActiveLC):
    mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas), mpUncertainty(pUncertainty), mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mRunType(RunType), mpMatchedKF(NULL), mLastLoopKFid(0), mbRunningGBA(false), mbFinishedGBA(true), mbStopGBA(false), mpThreadGBA(NULL), mbFixScale(bFixScale), mnFullBAIdx(0), mnLoopNumCoincidences(0), mnMergeNumCoincidences(0), mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC)
{
            ...
            if(bFindedRegion)
            {
                ...
                if(mbLoopDetected)
                {
                    ...
                    if (bGoodLoop) {
                        ...
                        cerr<<"mvpLoopMapPoints"<<mvpLoopMapPoints.size()<<"mvpLoopMPs"<<mvpLoopMPs.size()<<endl;
                    ...
                    KF->mnId = 20000000; //Indicate the start of the transmission
                    ...
                    msg = oos.str();
                    oos.clear();
                    mpLocalMapper->server_downlink_queue.enqueue(msg);
                
                    KFsData.clear();
                    vector<KeyFrame*> current_local_map = mpCurrentKF->GetMap()->GetAllKeyFrames();
                    ...
                    {
                        KeyFrame* tKF = *mit;
                        mpLocalMapper->PreSaveKFandMP(tKF);
                        std::ostringstream os;
                        boost::archive::text_oarchive oa(os);
                        oa << tKF;
                        KFsData.push_back(os.str());
