    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    getline(cin, mRunType);
    
        mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpAtlas, mpUncertainty, mpKeyFrameDatabase, strSettingsFile, mSensor, settings_, strSequence);

    //Initialize the Local Mapping thread and launch
    /////////////////////////////////CommSLAM///////////////////////////////////
    mpLocalMapper = new LocalMapping(this, mpAtlas, mpKeyFrameDatabase, mpVocabulary, mpUncertainty, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD, mRunType, strSequence);
    ...
    ////////////////////////////////CommSLAM//////////////////////////////////////
    mpLoopCloser = new LoopClosing(mpAtlas, mpUncertainty, mpKeyFrameDatabase, mpVocabulary, mRunType, mSensor!=MONOCULAR, activeLC); // mSensor!=MONOCULAR);
...
////////////////////CommSLAM////////////////////
void System::SaveUncertainty(const string &filename)
{
    cout<<endl<<"Saving uncertainty values to "<<filename<<"..."<<endl;
    ofstream f;
    f.open(filename.c_str());
    vector<float> LocalDOptimal=mpUncertainty->mvLocalDOptimal;
    vector<float> GlobalDOptimal=mpUncertainty->mvGlobalDOptimal;
    vector<float> LocalInertialDOptimal=mpUncertainty->mvLocalInertialDOptimal;
    vector<float> GlobalInertialDOptimal=mpUncertainty->mvGlobalInertialDOptimal;
    vector<float> MergeVisualDOptimal=mpUncertainty->mvMergeVisualDOptimal;
    for (vector<float>::iterator iter=LocalDOptimal.begin();iter!=LocalDOptimal.end();iter++)
    {
        f<<"Local Dopt"<<(*iter)<<endl;
    }
    for (vector<float>::iterator iter=GlobalDOptimal.begin();iter!=GlobalDOptimal.end();iter++)
    {
        f<<"Global Dopt"<<(*iter)<<endl;
    }
    for (vector<float>::iterator iter=LocalInertialDOptimal.begin();iter!=LocalInertialDOptimal.end();iter++)
    {
        f<<"Local Inertial Dopt"<<(*iter)<<endl;
    }
    for (vector<float>::iterator iter=GlobalInertialDOptimal.begin();iter!=GlobalInertialDOptimal.end();iter++)
    {
        f<<"Global Inertial Dopt"<<(*iter)<<endl;
    }
    for (vector<float>::iterator iter=MergeVisualDOptimal.begin();iter!=MergeVisualDOptimal.end();iter++)
    {
        f<<"Merge Visual Dopt"<<(*iter)<<endl;
    }
}
...
void System::ReceiveUplink()
{
    ...
    while(1)
    {
        ...
        if(mpLocalMapper->server_uplink_queue.try_dequeue(msg))
        {
            ...
            try
            {
                ...
                if (tKF->mnId == 10000000)
                {
                 
                    return;
                }
                
                mpLocalMapper->InsertKeyFrame(tKF);
             
                
            }
            catch(boost::archive::archive_exception e)
            {
                cout << "uplink transmission, server error: " << e.what() << endl;
                    //return;
            }
            ...
            for(int i=0; i<(int)mapVec.size(); i++)
            {
                ...
                try
                {
                    std::stringstream iiis(mapVec[i]);
            ...
            if (StoreKFid.size()!= 0)
            {
                    ...
                    mpAtlas->GetCurrentMap()->mspKeyFrames.clear();
...
void System::Exit()
{
    ...
    std::string msg;
    msg = os.str();
    mpLocalMapper->client_uplink_queue.enqueue(msg);
}
} //namespace ORB_SLAM
