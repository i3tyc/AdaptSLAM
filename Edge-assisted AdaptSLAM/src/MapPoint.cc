...
MapPoint::MapPoint():
    ...
    mTrackProjX(0),mTrackProjY(0),mTrackDepth(0),mTrackDepthR(0),mTrackProjXR(0),mTrackProjYR(0),mbTrackInView(0),mbTrackInViewR(0),mnTrackScaleLevel(0),mnTrackScaleLevelR(0),mTrackViewCos(0), mTrackViewCosR(0),
    mpReplaced(static_cast<MapPoint*>(NULL))
{
    mpReplaced = static_cast<MapPoint*>(NULL);
    mPosGBA = Eigen::Vector3f ();
}
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap),    mTrackProjX(0),mTrackProjY(0),mTrackDepth(0),mTrackDepthR(0),mTrackProjXR(0),mTrackProjYR(0),mbTrackInView(0),mbTrackInViewR(0),mnTrackScaleLevel(0),mnTrackScaleLevelR(0),mTrackViewCos(0), mTrackViewCosR(0),
    mnOriginMapId(pMap->GetId())
{
    SetWorldPos(Pos);
    
    ///////CommSLAM//////////
    mBackupObservationsId1=std::map<long unsigned int, int> ();
    mBackupObservationsId2=std::map<long unsigned int, int> ();
    mDescriptor=cv::Mat ();
    mBackupRefKFId=-1;
    mBackupReplacedId=-1;
    mPosGBA = Eigen::Vector3f ();
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId()),mfMinDistance(0), mfMaxDistance(0),    mTrackProjX(0),mTrackProjY(0),mTrackDepth(0),mTrackDepthR(0),mTrackProjXR(0),mTrackProjYR(0),mbTrackInView(0),mbTrackInViewR(0),mnTrackScaleLevel(0),mnTrackScaleLevelR(0),mTrackViewCos(0), mTrackViewCosR(0)
{
    SetWorldPos(Pos);
...
//////////////////////////CommSLAM///////////////////////
void MapPoint::PostLoadReplace(MapPoint* pMP)
{
...
{
    //////////////////CommSLAM///////////////
    //mnBackUpHostKFId = mpHostKF->mnId;
...
////////CommSLAM///////////
void MapPoint::PostLoad(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid, KeyFrame* kf)
{
    /////////////////////////CommSLAM/////////////////////////
    //mpHostKF=mpKFid[mnBackUpHostKFId];
    
    //////CommSLAM////////////
    if(mpKFid.find(mBackupRefKFId) != mpKFid.end())
    ...
    for(map<long unsigned int, int>::const_iterator it = mBackupObservationsId1.begin(), end = mBackupObservationsId1.end(); it != end; ++it)
    {
        ...
        if(mpKFid.find(it->first) != mpKFid.end())
        {
            ...
            //end_time = std::chrono::system_clock::to_time_t(endtime);
 
            //std::cout << "map point time: " << elapsed_seconds.count() << "s"<< std::endl;
        map<long unsigned int, int>::const_iterator it2 = mBackupObservationsId2.find(it->first);
        indexes = tuple<int,int>(it->second,it2->second);
        if(pKFi)
        {
